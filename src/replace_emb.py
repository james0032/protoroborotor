...
newpl = pd.scan_csv(os.path.join(BASE_PATH, "projected_entity_embeddings.tsv"), separator='\t', has_header=False)
...
#)
#new.to_csv(os.path.join(BASE_PATH, "formatted_projected_entity_embeddings.tsv"), sep='\t', index=False)
...
newpl_count = newpl.select(pl.count()).collect().item()
...
for i in range(num_partitions):
    ...
    #end = min((i + 1) * partition_size, len(newpl))
    df_slice = df.slice(start, partition_size)

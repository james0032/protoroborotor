...
#df = df.drop("topological_embedding") # Don't drop it when join
...
null_ids = (
    df.filter(pl.col("topological_embedding_right").is_null())
           .select("id")
           .collect()["id"].to_list()
)
...
df = (
    df.join(rand_df.lazy(), on="id", how="left", suffix="_rand")
           .with_columns(
               pl.when(pl.col("topological_embedding_right").is_null())
                 .then(pl.col("topological_embedding_rand"))
                 .otherwise(pl.col("topological_embedding_right"))
                 .alias("topological_embedding_right")
           )
           .with_columns(
                pl.concat_list([pl.col("topological_embedding"), pl.col("topological_embedding_right")])
                .alias("embedding")
           )
           .drop(["topological_embedding_rand", "topological_embedding", "topological_embedding_right"])
           .rename({"embedding": "topological_embedding"})
)
